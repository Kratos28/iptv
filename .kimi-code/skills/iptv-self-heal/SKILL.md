...
- `Dockerfile` / `docker-entrypoint.sh` / `fetch.sh`：自托管部署（python:3.13-alpine + busybox crond，纯标准库无浏览器依赖）。启动即拉取一轮，之后每天 12 次（每 2 小时，北京时间；咪咕地址寿命实测正好 3 小时，过期返回 HTTP 605，4 小时间隔会出现全组失效空窗），8111 端口提供 `iptv.txt`。
...
- `EXTRA_CHANNELS`（iptv.py 内）：手工维护的补充频道候选地址列表，失效地址主要在这里修。目前含地方频道（广东民生）、港澳（凤凰中文台/凤凰资讯台/凤凰香港台、翡翠台/翡翠台4K）。补充频道复验失败时会依次回退其余候选地址，仍失败才剔除。**vicp.fun 等组播代理地址不进 EXTRA_CHANNELS，统一由 whiteList.txt 维护。**已知坑：vicp.fun / hk.188766.xyz 等 28.0.0.x 系域名在腾讯云 DNS 可能解析失败（Errno -3），广东民生因此备了直连 IP 候选；jdshipin 中继偶发 Connection reset，复验回退可消化。
- 白名单（仓库根目录 `whiteList.txt`，`IPTV_WHITELIST` 可改路径，Docker 镜像已 COPY 到 /app）：名单中的频道跳过测速和复验直接收录进订阅——频道已有实测通过的源时白名单地址作为同名备用行追加（排在主地址之后），频道缺失（未收到或复验被剔除）时白名单地址作为主地址兜底，名单中的频道必定出现在订阅里。格式每行一条 `频道名,URL`（同名多行表示同频道多个地址），分组按频道名自动归类（央视/卫视/地方），也可用 `分组,#genre#` 行显式指定。适用于已知稳定播放但运行环境实测不通的地址（如 vicp.fun 广州台组播代理，服务器上偶发 503）。限速合并轮同样生效。
- `SUPPLEMENT_CHANNELS`（iptv.py 内）：从 iptv-org 公共源补全的央卫视频道，按 tvg-id 前缀匹配。
- 外部高清源（iptv.py 内 `HD_EXTRA_SOURCES`，每项 "地址|最低高度"，默认 live.zbds.top/tv/iptv4.txt|320 + develop202 migu_video interface.txt|320，可用 `IPTV_HD_EXTRA_URLS` 覆盖或置空关闭）：收集已有频道达到最低高度的地址（纯标准库实现分辨率探测：m3u8 读 RESOLUTION 标签，没有则下载分片解析 H.264 SPS；H.265/fMP4 暂不解析按不通过处理），实测流畅后写入同名多行：央视频道/卫视频道的咪咕主源固定排第一位，其余源按实测高度从高到低；其他频道 720p 及以上按实测高度从高到低排在频道源首位，低于 720p 的列在主地址之后。限速合并轮跳过探测，合并时旧订阅里的同名高清行原样保留。zbds 多为运营商/酒店组播转 HTTP 源，失效属常态，不用逐个修。
