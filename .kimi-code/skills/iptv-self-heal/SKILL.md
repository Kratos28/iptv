...
- `yangshipin.py`：央视频备用源抓取（唯一非标准库依赖：Playwright + Chromium）。央视频官方接口签名由 WASM 计算、一次性有效，无法纯 HTTP 重放，故用无头浏览器打开直播页截取 m3u8（vkey 路径，去签名参数可播，有效期约 4 小时）。央视频 CDN 按抓取地分发（境外会拿到 outlivecloud，国内无法播放），vkey 与 CDN 无关（换 host 仍可播），脚本统一改写 host 到国内腾讯 CDN（hlslive-tx-cdn.ysp.cctv.cn）。输出 `yangshipin.json`（gitignored），`iptv.py` 读取后补齐咪咕/公共源都缺的央视、卫视频道（官方 CDN 免测速，能播即写入，复验同样免测速）。频道清单来自 `capi.yangshipin.cn/api/oms/pc/page/PG00000004`（protobuf，正则解析名字+9 位 pid）。
