...
自动抓取咪咕视频 App 的电视直播源（央视、卫视、地方台等），逐个实测验证播放流畅度，生成 txt 格式订阅文件。GitHub Actions 每天定时更新 3 次（北京时间 08:03 / 13:17 / 19:41）。
...
- GitHub 托管的 runner 位于境外，咪咕 CDN 对部分海外 IP 限速，可能导致验证通过率低；如需更高可用性，可自行部署在国内网络环境运行。
